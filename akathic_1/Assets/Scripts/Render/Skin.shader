...
#include "AutoLight.cginc"
		...
		float4 frag(v2f i) :COLOR
		{
			...
			*/
			...
			float3 c = tex2D(_MainTex, i.uv_MainTex) * 64;
			c += tex2D(_BlurTex1, i.uv_MainTex) * 32;
			c += tex2D(_BlurTex2, i.uv_MainTex) * 16;
			c += tex2D(_BlurTex3, i.uv_MainTex) * 8;
			c += tex2D(_BlurTex4, i.uv_MainTex) * 4;
			c += tex2D(_BlurTex5, i.uv_MainTex) * 2;
			c /= 128;
			...
			*/
			...
			float ao = tex2D(_AOTex,i.uv_MainTex).r;
